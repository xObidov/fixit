...
app.post('/login', (req, res) => {
  ...
  if (!email || !password) return res.status(400).json({ message: 'Email va parol kiritilishi kerak' });
...
app.put('/user/orders/:id/userstatus', authenticateToken, (req, res) => {
  ...
  const { userstatus } = req.body; // "confirmed" yoki "canceled"

  if (!['confirmed', 'canceled'].includes(userstatus)) {
    return res.status(400).json({ message: "Faqat 'confirmed' yoki 'canceled' qiymati bo'lishi mumkin" });
  }
  ...
  db.get('SELECT * FROM orders WHERE id = ? AND userId = ?', [orderId, userId], (err, order) => {
    ...
    if (order.status !== 'fixed' || order.userstatus !== 'waiting')
      return res.status(400).json({ message: 'Faqat statusi fixed va userstatus waiting bo‘lgan orderlar o‘zgartiriladi' });

    db.run('UPDATE orders SET userstatus = ? WHERE id = ?', [userstatus, orderId], function(err) {
      if (err) return res.status(500).json({ message: 'Yangilashda xatolik', error: err.message });
      res.json({ message: `Buyurtma userstatusi ${userstatus} ga o‘zgartirildi` });
    });
  });
});
...
app.put('/fixer/orders/:id', authenticateToken, requireFixer, (req, res) => {
  ...
  db.get('SELECT status FROM orders WHERE id = ?', [orderId], (err, row) => {
    ...
    if (row.status !== 'confirmed')
      return res.status(400).json({ message: "Faqat confirmed bo'lgan buyurtmalarni yangilash mumkin" });
    ...
    db.run(
      ...
      function (err) {
        ...
        res.json({ message: "Buyurtma yangilandi va status 'fixed' ga o'zgardi", orderId });
      }
    );
  });
});
    ...
    function insertOrder(uid, passwordToSend) {
      db.run(
        `INSERT INTO orders (userId, description, phone, serviceTime, customerType, name, district, status)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
        [uid, description, phone, serviceType, customerType, name, district, 'pending'],
        function (err) {
